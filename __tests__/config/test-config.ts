...
export const TEST_CONFIG = {
  ...
  n8n: {
    ...
    webhookUrl:
      process.env.TEST_N8N_WEBHOOK_URL ||
...
export const validateTestEnvironment = () => {
  ...
  process.env.SUPABASE_SERVICE_ROLE_KEY =
    TEST_CONFIG.database.testServiceRoleKey;
};
...
export const generateTestData = {
  ...
  contentItems: (
    campaignId: string,
    count: number = TEST_CONFIG.testData.contentItems.count
  ) => {
    return Array.from({ length: count }, (_, i) => ({
      id: `test-content-${i + 1}`,
  ...
  contentVersions: (
    ...
    count: number = TEST_CONFIG.testData.contentItems.versionsPerItem
  ) => {
    return Array.from({ length: count }, (_, i) => ({
      id: `test-version-${i + 1}`,
  ...
  workflowExecutions: (
    contentItemId: string,
    count: number = TEST_CONFIG.testData.workflowExecutions.count
  ) => {
    return Array.from({ length: count }, (_, i) => ({
